...
try {
  ...
  await page.evaluate(() => {
    ...
    s.spawnTroop(6.5, 13, 'golem', 'PLAYER', 0, 1)
    s.spawnTroop(9.5, 13, 'golem', 'PLAYER', 0, 2)
    s.spawnTroop(12.5, 13, 'golem', 'PLAYER', 0, 3)
    s.spawnTroop(6.5, 16.4, 'golem', 'ENEMY', 0, 1)
    s.spawnTroop(9.5, 16.4, 'golem', 'ENEMY', 0, 2)
    s.spawnTroop(12.5, 16.4, 'golem', 'ENEMY', 0, 3)
    // headings grid (L2 player): 8 headings, two rows of 4
    for (let k = 0; k < 4; k++) s.spawnTroop(6 + k * 3.2, 21, 'golem', 'PLAYER', 0, 2)
    for (let k = 4; k < 8; k++) s.spawnTroop(6 + (k - 4) * 3.2, 24.4, 'golem', 'PLAYER', 0, 2)
    // demo golem for walk/slam closeups (map is 25x25 — stay on it)
    s.spawnTroop(19.5, 22.5, 'golem', 'PLAYER', 0, 3)
