...
`ATTACK_SIMULATION_VERSION` is **9**. V8 evaluates each Siege Tower's recorded
...
preserve the historical unconditional credit. V9 credits a bounded share of
Spike Launcher hazard damage and gives every declarative suicide troop one
settlement detonation; stored v8 keeps the former direct-shot/cadence math.
The packed asset count and exact icon set are pinned by
