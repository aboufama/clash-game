...
// Dragon's Breath: design B ("The Ember-Wyrm Reliquary") won the clean-room
// tournament and is the SHIPPED canonical body — called directly, no registry
// indirection.
import { drawDragonsBreathB } from './redesign/Dragons_breathB';
...
export class BuildingRenderer {
    ...
    /**
     * DRAGON'S BREATH — "The Ember-Wyrm Reliquary" (design B, tournament
     * winner 2026-07-22, promoted canonical — the cannon/golem/deadwood/
     * barracks precedent). The 2-variant clean-room round is RESOLVED:
     * design A was rejected and deleted; the round left the DesignRegistry.
     * The full body — dormant shut dragon mouth, deploy01 rise choreography
     * (jaw apron, rearing-then-sinking skull-lid, 16-bearing rotating rocket
     * battery, serpentine volley clock) — lives in
     * ./redesign/Dragons_breathB.ts and is called directly.
     */
    static drawDragonsBreath(graphics: Phaser.GameObjects.Graphics, c1: Phaser.Math.Vector2, c2: Phaser.Math.Vector2, c3: Phaser.Math.Vector2, c4: Phaser.Math.Vector2, center: Phaser.Math.Vector2, alpha: number, tint: number | null, building ?: any, baseGraphics ?: Phaser.GameObjects.Graphics, gridX: number = 0, gridY: number = 0, time: number = 0, skipBase: boolean = false, onlyBase: boolean = false) {
        drawDragonsBreathB(
            graphics, c1, c2, c3, c4, center, alpha, tint,
            building, baseGraphics, gridX, gridY, time, skipBase, onlyBase
        );
    }
