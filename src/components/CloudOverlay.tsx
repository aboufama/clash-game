...
const PX = 3;             // CSS pixels per cloud pixel
const CLOSE_MS = 700;     // bank roll-in
const OPEN_MS = 800;      // bank part (App hides the overlay just after)
...
// two packed rows, then the big sunlit rampart at the seam.
const HAZE = '#eaf0f6';
const ROW_B: PuffPalette = { shadow: '#b0c0d3', body: '#d8e2ec', crown: '#e9eff5' };
const ROW_A: PuffPalette = { shadow: '#b6c6d8', body: '#e2eaf1', crown: '#f3f7fb' };
const CREST: PuffPalette = { shadow: '#b2c2d4', body: '#dbe4ee', crown: '#edf2f7' };
const FORE: PuffPalette = { shadow: '#bfcedd', body: '#e9eff5', crown: '#f9fbfd' };
 ...
 */
function drawBank(ctx: CanvasRenderingContext2D, W: number, H: number, T: number, dir: -1 | 1, off: number) {
  ...
  // --- the deep bank: one flat near-white fill (parallax 0.9). The
  // interior is deliberately EMPTY — every sculpted shape belongs to the
  // ranks at the seam, exactly like the world's fog. ---
  ctx.save();
  ctx.translate(dir * off * 0.9, 0);
  ctx.fillStyle = HAZE;
  ...
  for (let y = -32; y <= H + 32; y += step) {
    ...
    const w = 92 + (h % 36);
    const x = front + dir * ((h >> 3) % 14) - dir * w * 0.12;
...
export function CloudOverlay({
  ...
  useEffect(() => {
    ...
    const draw = (now: number) => {
      ...
      // Cover: 0 = banks parked off-screen, 1 = view fully swallowed.
      let cover = loadingRef.current ? 1 : easeOutCubic(Math.min(1, (now - mountedAtRef.current) / CLOSE_MS));
      const oAt = openingAtRef.current;
      if (oAt !== null) {
        cover *= 1 - easeInCubic(Math.min(1, (now - oAt) / OPEN_MS));
      }
      ctx.clearRect(0, 0, W, H);
      const T = now * 0.001;
      const off = (1 - cover) * (FRONT * W + 90);
      drawBank(ctx, W, H, T, -1, off);
      drawBank(ctx, W, H, T, 1, off);
    };
    raf = requestAnimationFrame(draw);
